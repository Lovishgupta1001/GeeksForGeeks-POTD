...
public:
                                    ...
                                    vector<vector<int>> graph)
    {
        ...
        for (int subset = 0; subset < (1 << N); subset++) // check for all 2^N subsets
        {
            ...
            {
                if (!(subset & (1 << curr)))
                    continue; // skip if current node is not part of subset

                for (int prev = 0; prev < N; prev++) // going through all previous nodes
                {
                    if (curr == prev || arr[curr] == arr[prev])
                        continue; // skip if previous node is same as current node

                    if (!(subset & (1 << prev)))
                        continue; // skip if previous node is not in subset

                    if (!adj[arr[prev] - 1][arr[curr] - 1])
                        continue; // skip if no edge between current and previous node
